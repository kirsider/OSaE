...
LRESULT CALLBACK WndProc(
    ...
    switch(msg) {
        ...
        case WM_CREATE: {
            ...
            text_box = CreateWindow(L"EDIT", L"", WS_PARAMS, 80, 40, 510, 30, hwnd, (HMENU) ID_TEXT_BOX, NULL, NULL);
            CheckForErrors(hwnd, text_box, GET_VARIABLE_NAME(text_box));

            list_box_1 = CreateWindow(L"LISTBOX", L"", WS_PARAMS, 80, 100, 250, 300, hwnd, (HMENU) ID_LIST_BOX_1, NULL, NULL);
            CheckForErrors(hwnd, list_box_1, GET_VARIABLE_NAME(list_box_1));

            list_box_2 = CreateWindow(L"LISTBOX", L"", WS_PARAMS, 470, 100, 250, 300, hwnd, (HMENU) ID_LIST_BOX_2, NULL, NULL);
            CheckForErrors(hwnd, list_box_2, GET_VARIABLE_NAME(list_box_2));

            add_button = CreateWindow(L"Button", L"Add", WS_PARAMS, 620, 40, 100, 30, hwnd, (HMENU) ID_ADD_BUTTON, NULL, NULL);
            CheckForErrors(hwnd, add_button, GET_VARIABLE_NAME(add_button));

            clear_button = CreateWindow(L"Button", L"Clear", WS_PARAMS, 155, 420, 100, 30, hwnd, (HMENU) ID_CLS_BUTTON, NULL, NULL);
            CheckForErrors(hwnd, clear_button, GET_VARIABLE_NAME(clear_button));

            del_button = CreateWindow(L"Button", L"Delete", WS_PARAMS, 545, 420, 100, 30, hwnd, (HMENU) ID_DEL_BUTTON, NULL, NULL);
            CheckForErrors(hwnd, del_button, GET_VARIABLE_NAME(del_button));

            to_r_button = CreateWindow(L"Button", L"To Right", WS_PARAMS, 350, 215, 100, 30, hwnd, (HMENU) ID_TO_R_BUTTON, NULL, NULL);
